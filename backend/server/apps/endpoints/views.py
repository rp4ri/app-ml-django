...
class StopABTestView(views.APIView):
    def post(self, request, ab_test_id, format=None):
        ...
        try:
            ...
            # alg #1 accuracy
            # Nota: en el dato de fecha de cracion tengo que anadir 4 horas
            ...
            all_responses_1 = MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithm_1, created_at__gt = ab_test.created_at, created_at__lt = date_now+datetime.timedelta(hours=4)).count()
            #print("all_responses_1", all_responses_1)
            ...
            correct_responses_1 = MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithm_1, created_at__gt = ab_test.created_at, created_at__lt = date_now+datetime.timedelta(hours=4), response=F('feedback')).count()
            #print("correct_responses_1", correct_responses_1)
            ...
            all_responses_2 = MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithm_2, created_at__gt = ab_test.created_at, created_at__lt = date_now+datetime.timedelta(hours=4)).count()
            ...
            correct_responses_2 = MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithm_2, created_at__gt = ab_test.created_at, created_at__lt = date_now+datetime.timedelta(hours=4), response=F('feedback')).count()
